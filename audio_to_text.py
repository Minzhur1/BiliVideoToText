...
def detect_and_transcribe(audio_path, model_size="small"):
    ...
    try:
        if HAS_FASTER_WHISPER:
            ...
            return _transcribe_original(audio_path, model_size)
...
def _transcribe_original(audio_path, model_size="small"):
    """
    原版Whisper实现（备选）
    """
    import whisper
    ...
    print(f"✅ 转录完成！语言: {detected_lang}, 长度: {len(text)}字符")
...
def test():
    ...
    if os.path.exists("test.wav"):
        lang, text = detect_and_transcribe("test.wav", "small")
        if text:
            print(f"\n预览: {text[:100]}...")
    else:
